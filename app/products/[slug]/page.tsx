    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-rose-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
          {/* Product Images */}
          <div className={`space-y-4 transition-all duration-1000 ${isVisible ? "animate-slide-in-left" : ""}`}>
                ...
                {productImages.map((image, index) => (
                  <button
                    ...
                    className={`w-20 h-20 rounded-lg overflow-hidden border-2 transition-all duration-300 hover:scale-105 ${
                      selectedImage === index ? "border-rose-600 shadow-lg" : "border-gray-200 hover:border-gray-300"
                    }`}
                  >
                    <img
          ...
          {/* Product Info */}
          <div className={`space-y-6 transition-all duration-1000 ${isVisible ? "animate-slide-in-right" : ""}`}>
            <div>
              <div className="flex items-center gap-2 mb-3">
                <Badge variant="secondary" className="animate-bounce-gentle">
                  {product.category}
                </Badge>
                {product.isNew && <Badge className="bg-green-600 animate-pulse-glow">New</Badge>}
                {product.onSale && <Badge className="bg-red-600 animate-pulse-glow">Sale</Badge>}
              </div>
                  ...
                  {[...Array(5)].map((_, i) => (
                    <Star
                      ...
                      className={`w-6 h-6 transition-all duration-300 hover:scale-125 ${
                        i < product.rating ? "fill-yellow-400 text-yellow-400 animate-twinkle-slow" : "text-gray-300"
                      }`}
                    />
                  ))}
                </div>
              ...
              <div className="flex items-center gap-4 mb-6">
                <span className="text-4xl font-bold text-rose-600 animate-pulse-slow">${product.price}</span>
            ...
            <div className="animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
              <h3 className="font-semibold text-gray-900 mb-3 text-lg">Suitable for:</h3>
              <div className="flex flex-wrap gap-2">
                {product.skinTypes.map((type, index) => (
                  <Badge
                    key={type}
                    variant="outline"
                    className="border-rose-300 text-rose-700 hover:bg-rose-50 transition-all duration-300 hover:scale-105 animate-fade-in-up"
            ...
            <div className="animate-fade-in-up" style={{ animationDelay: "0.4s" }}>
              <h3 className="font-semibold text-gray-900 mb-3 text-lg">Addresses:</h3>
              <div className="flex flex-wrap gap-2">
            ...
            <div className="flex items-center gap-4 animate-fade-in-up" style={{ animationDelay: "0.6s" }}>
              ...
              <Button
                ...
                className="flex-1 bg-rose-600 hover:bg-rose-700 text-lg py-6 ripple-mega transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl"
                size="lg"
              >
              ...
              <Button
                variant="outline"
                size="lg"
                className="py-6 px-6 border-2 border-rose-300 text-rose-700 hover:bg-rose-50 transform hover:scale-105 transition-all duration-300 bg-transparent"
              >
        ...
        <div className="mb-16 animate-fade-in-up">
          <Tabs defaultValue="description" className="w-full">
            <TabsList className="grid w-full grid-cols-4 bg-white shadow-lg rounded-xl p-2">
              <TabsTrigger value="description" className="rounded-lg transition-all duration-300">
                Description
              </TabsTrigger>
              <TabsTrigger value="ingredients" className="rounded-lg transition-all duration-300">
                Ingredients
              </TabsTrigger>
              <TabsTrigger value="usage" className="rounded-lg transition-all duration-300">
                How to Use
              </TabsTrigger>
              <TabsTrigger value="reviews" className="rounded-lg transition-all duration-300">
                Reviews
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="ingredients" className="mt-8">
              <Card className="shadow-xl border-0">
                <CardContent className="p-8">
                  <h3 className="font-semibold mb-6 text-xl">Key Ingredients:</h3>
                  <div className="space-y-4">
                    {product.keyIngredients?.map((ingredient, index) => (
                      <div key={index} className="animate-fade-in-up" style={{ animationDelay: `${index * 0.1}s` }}>
                        <h4 className="font-medium text-gray-900 text-lg">{ingredient.name}</h4>
                        <p className="text-gray-600">{ingredient.benefit}</p>
                      </div>
                    )) || (
                      <p className="text-gray-600 text-lg">
                        This product contains carefully selected ingredients that are gentle yet effective for your skin
                        type.
                      </p>
                    )}
                  </div>
              ...
              <Card className="shadow-xl border-0">
                <CardContent className="p-8">
                  <div className="space-y-6">
                    <div>
                      <h3 className="font-semibold mb-3 text-xl">How to Use:</h3>
                    ...
                    <div>
                      ...
                      <p className="text-gray-700 text-lg">{product.bestUsed || "Morning and evening"}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-8">
                  <div className="space-y-8">
                    <div className="flex items-center gap-6">
                      <div className="text-5xl font-bold animate-pulse-slow">{product.rating}</div>
                      <div>
                        <div className="flex items-center gap-1 mb-2">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`w-6 h-6 ${i < product.rating ? "fill-yellow-400 text-yellow-400 animate-twinkle-slow" : "text-gray-300"}`}
                            />
                          ))}
                        </div>
                        <p className="text-gray-600 text-lg">Based on {product.reviews} reviews</p>
                      </div>
                    </div>
        ...
        {relatedProducts.length > 0 && (
          <div className="animate-fade-in-up">
            <h2 className="text-3xl font-serif font-bold text-gray-900 mb-8 text-center">You Might Also Like</h2>
                    ...
                    <div className="p-4">
                      <div className="flex items-center gap-1 mb-2">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            key={i}
                            className={`w-4 h-4 ${i < relatedProduct.rating ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`}
                          />
                        ))}
                        <span className="text-sm text-gray-500 ml-1">({relatedProduct.reviews})</span>
                      </div>
                      <h3 className="font-semibold text-gray-900 mb-1 group-hover:text-rose-600 transition-colors duration-300">
                        {relatedProduct.name}
                      </h3>
                      <p className="text-sm text-gray-600 mb-3">{relatedProduct.category}</p>
